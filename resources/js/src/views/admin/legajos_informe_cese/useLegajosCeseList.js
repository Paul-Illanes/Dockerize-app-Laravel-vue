...
export default function useLegajosCeseList() {
  // Use toast
  const toast = useToast()

  const refLegajosCeseListTable = ref(null)
  ...
  const tableColumns = [
    ...
    { key: 'ultimo_dia_labor', sortable: true },
  ...
  const dataMeta = computed(() => {
    const localItemsCount = refLegajosCeseListTable.value ? refLegajosCeseListTable.value.localItems.length : 0
    return {
  ...
  const refetchData = () => {
    refLegajosCeseListTable.value.refresh()
  ...
  const fetchCeses = (ctx, callback) => {
    store
      .dispatch('app-legajo-cese/fetchLegajoCese', {
        q: searchQuery.value,
