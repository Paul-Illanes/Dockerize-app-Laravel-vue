<template>
    ...
    <b-card no-body>
        <div class="m-2">
            ...
            <b-row>
                ...
                >
                    ...
                    <b-button
                        v-if="$can('contrato-create', 'ACL')"
                        variant="primary"
                        :to="{ name: 'admin-contratos-add' }"
                    >
                        Agregar
                    </b-button>
                </b-col>
        ...
        >
            ...
            <!-- Column: Id -->
            <template #cell(status_firma)="data">
                <b-dropdown
                    toggle-class="p-0"
                    no-caret
                    pill
                    :variant="changeColorFirma(data.item.status_firma)"
                    class="text-center"
                >
                    <template #button-content>
                        <b-badge
                            ...
                            :variant="changeColorFirma(data.item.status_firma)"
                        >
                            {{ changeStatusFirma(data.item.status_firma) }}
                        </b-badge>
                    </template>

                    <b-dropdown-item :disabled="data.item.status_firma == 1">
                        <feather-icon class="text-success" icon="CheckIcon" />
                        <span
                            @click="updateStatusFirma(1, data.item.id)"
                            class="align-middle ml-50"
                            >Firmado</span
                        >
                    </b-dropdown-item>
                    <b-dropdown-item :disabled="data.item.status_firma == 0">
                        <feather-icon
                            class="text-warning"
                            icon="PauseCircleIcon"
                        />
                        <span
                            @click="updateStatusFirma(0, data.item.id)"
                            class="align-middle ml-50"
                            >Pendiente</span
                        >
                    </b-dropdown-item>
            ...
            <!-- Column: Status -->
            <template #cell(status_contrato)="data">
                <b-dropdown
                    toggle-class="p-0"
                    no-caret
                    pill
                    :variant="changeColor(data.item.status_contrato)"
                    class="text-center"
                >
                    <template #button-content>
                        <b-badge
                            ...
                            :variant="changeColor(data.item.status_contrato)"
                        >
                            {{ changeStatus(data.item.status_contrato) }}
                        </b-badge>
                    </template>

                    <b-dropdown-item :disabled="data.item.status_contrato == 1">
                        <feather-icon class="text-success" icon="CheckIcon" />
                        <span
                            @click="updateStatusContrato(1, data.item.id)"
                            class="align-middle ml-50"
                            >Vigente</span
                        >
                    </b-dropdown-item>
                    <b-dropdown-item :disabled="data.item.status_contrato == 0">
                        <feather-icon
                            class="text-warning"
                            icon="PauseCircleIcon"
                        />
                        <span
                            @click="updateStatusContrato(0, data.item.id)"
                            class="align-middle ml-50"
                            >Extinto</span
                        >
                    </b-dropdown-item>
            ...
            <template #cell(actions)="data">
                <div class="text-nowrap">
                    ...
                    /> -->
                    <feather-icon
                        v-if="$can('contrato-edit', 'ACL')"
                        @click="
                            $router.push({
                                name: 'admin-contratos-edit',
                                params: { contratoId: data.item.id },
                            })
                        "
...
import useContratoList from "./useContratoList";
import contratoStoreModule from "./contratoStoreModule";
...
export default {
    ...
    methods: {
        confirmDelete(id) {
            this.$swal({
                ...
                if (result.value) {
                    this.$http
                        .post("/api/auth/contrato/delete/" + id)
                        .then((res) => {
                            this.$toast({
        ...
        changeStatus(dato) {
            if (dato == "0") return "Extinto";
            if (dato == "1") return "Vigente";
        },
        changeStatusFirma(dato) {
            if (dato == "0") return "Pendiente";
            if (dato == "1") return "Firmado";
        },
        changeColor(dato) {
            if (dato == "0") return "warning";
            if (dato == "1") return "primary";
        },
        changeColorFirma(dato) {
            if (dato == "0") return "warning";
            if (dato == "1") return "primary";
        },
        updateStatusFirma(status, id) {
            this.$http
                .post("/api/auth/contrato/statusfirma/" + id, {
                    status: status,
                })
                .then((res) => {
                    // console.log(res);
        ...
        updateStatusContrato(status, id) {
            this.$http
                .post("/api/auth/contrato/statuscontrato/" + id, {
                    status: status,
                })
                .then((res) => {
                    // console.log(res);
    ...
    setup() {
        const CONTRATO_APP_STORE_MODULE_NAME = "app-contrato";

        // Register module
        if (!store.hasModule(CONTRATO_APP_STORE_MODULE_NAME))
            store.registerModule(
                CONTRATO_APP_STORE_MODULE_NAME,
                contratoStoreModule
            );
        // UnRegister on leave
        onUnmounted(() => {
            if (store.hasModule(CONTRATO_APP_STORE_MODULE_NAME))
                store.unregisterModule(CONTRATO_APP_STORE_MODULE_NAME);
        });

        const statusOptions = ["Vigente", "Extinto"];
